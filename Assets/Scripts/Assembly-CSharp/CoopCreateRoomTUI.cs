...
public class CoopCreateRoomTUI : MonoBehaviour, TUIHandler
{
	...
	public void Awake()
	{
		...
		TNetObject tnetObj = TNetConnection.Connection;
		tnetObj.SetOnDisconnect(OnContectingLost);
		tnetObj.SetOnJoinedRoom(OnRoomCreation);
	...
	public void HandleEvent(TUIControl control, int eventType, float wparam, float lparam, object data)
	{
		...
		{
			audioPlayer.PlayAudio("Button");
			string room_name = /*tnetObj.Myself.Name*/UnityEngine.Random.Range(0, 99999) + "|coop";
			thePassword = CreatePasswordPanel.GetComponent<TUITextField>().GetText();
	...
	private void OnRoomCreation()
	{
		...
		//{
			...
			//net_com.m_netUserInfo.SetNetUserInfo(true, GameApp.GetInstance().GetGameState().nick_name, int.Parse(PhotonNetwork.room.name.Split('|')[0]), PhotonNetwork.player.ID, 0, currentChosenMapId, 0, (int)GameApp.GetInstance().GetGameState().Avatar, GameApp.GetInstance().GetGameState().LevelNum);
			...
			TNetConnection.Connection.RemoveCallbacks();
			if (PhotonNetwork.isMasterClient)
			{
				ExitGames.Client.Photon.Hashtable table = new ExitGames.Client.Photon.Hashtable();
				table.Add("password", thePassword);
				table.Add("mapNumber", currentChosenScene);
