...
public class GameUIScriptNew : MonoBehaviour, TUIHandler, ITutorialGameUI
{
	...
	public void HandleEvent(TUIControl control, int eventType, float wparam, float lparam, object data)
	{
		...
		else if (control.name == "PauseButton" && eventType == 3)
		{
			...
			else if (gameScene.GamePlayingState == PlayingState.GameLose && gameState.gameMode == GameMode.Coop && msgBoxYesOrNo.GetComponent<MsgBoxDelegate>().m_type != MsgBoxType.Rebirth && msgBoxYesOrNo.GetComponent<MsgBoxDelegate>().m_type != MsgBoxType.CrystalRebirth && (gameScene as GameMultiplayerScene).SFS_Player_Arr.Values.Count > 0)
			{
				ShowPausePanel();
		...
		else if (control.name == "Msg_Yes_Button" && eventType == 3)
		{
			...
			case MsgBoxType.Rebirth:
			{
				...
				ServerEventSystem.Send(15, null);
	...
	private void OnDestroy()
	{
		...
		if (gameState != null && gameState.net_com != null)
		{
