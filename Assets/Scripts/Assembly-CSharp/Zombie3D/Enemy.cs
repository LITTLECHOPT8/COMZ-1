...
namespace Zombie3D
{
	public abstract class Enemy
	{
		...
		{
			if (GameApp.GetInstance().GetGameState().gameMode == GameMode.Coop && PhotonNetwork.isMasterClient)
			{
				player = null;
				if (((GameMultiplayerScene)GameApp.GetInstance().GetGameScene()).SFS_Player_Arr.Values.Count > 0)
				{
					int index = UnityEngine.Random.Range(0, ((GameMultiplayerScene)GameApp.GetInstance().GetGameScene()).SFS_Player_Arr.Values.Count);
					player = System.Linq.Enumerable.ToList(((GameMultiplayerScene)GameApp.GetInstance().GetGameScene()).SFS_Player_Arr.Values)[index];
				}
				else
				{
		...
		{
			if (GameApp.GetInstance().GetGameState().gameMode == GameMode.Coop && PhotonNetwork.isMasterClient && player.GetPlayerState() != null && player.GetPlayerState().GetStateType() == PlayerStateType.Dead)
			{
				SetTargetWithMultiplayer();
			}
		}
		...
		{
			if ((GameApp.GetInstance().GetGameState().gameMode == GameMode.Coop && !PhotonNetwork.isMasterClient) || player == null || !(Time.time - lastPathFindingTime > 0.5f))
			{
				return;
			}
			lastPathFindingTime = Time.time;
