...
namespace Zombie3D
{
	public static class EnermyFactory
	{
		...
		public static Object Instantiate(UnityEngine.Object obj, Vector3 position = default(Vector3), Quaternion rotation = default(Quaternion), string path = "")
		{
		...
		public static Enemy SpawnEnemy(int id, bool isElite, bool isPrey, bool isBoss, EnemyType enermyType, SpawnFromType fromWhere, Vector3 position)
		{
			...
			bool flag = false;
			if ((GameApp.GetInstance().GetGameState().gameMode == GameMode.Vs || GameApp.GetInstance().GetGameState().gameMode == GameMode.Coop) && Application.loadedLevelName.StartsWith("Zombie3D") && !PhotonNetwork.isMasterClient)
			{
				gameObject2 = GameObject.Find("E_" + id);
			}
			else
			{
				if (enermyType > EnemyType.E_COUNT)
				{
					...
					gameObject = Instantiate(Resources.Load(text)) as GameObject;
					gameObject2 = Instantiate(gameObject.GetComponent<AvataConfigScript>().Avata_Instance, position, Quaternion.identity, "photon/Zombies/" + gameObject.GetComponent<AvataConfigScript>().Avata_Instance.name.Replace("(Clone)", "")) as GameObject;
					gameObject2.layer = 9;
					flag = true;
				...
				else if (isElite)
				{
					...
					gameObject2 = Instantiate(gameObject, position, Quaternion.identity, "photon/Zombies/" + gameObject.name.Replace("(Clone)", "")) as GameObject;
					gameObject2.layer = 9;
					flag = true;
				}
