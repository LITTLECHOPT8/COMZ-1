	...
	public abstract class PlayerBonusState
	{
		...
		public bool CheckEnableEquipItem(Player player, ItemType itemType)
		{
			...
			else if (itemType == ItemType.Shield || itemType == ItemType.InstantStealth)
			{
				...
				if (transform.GetComponent<Renderer>().material.shader.name != "iPhone/SolidTexture" && transform.GetComponent<Renderer>().material.shader.name != "iPhone/SolidAndAlphaTextureEx")
				{
					return false;
				}
				if (player.GetWeapon().GetWeaponType() == WeaponType.Saw)
				{
					...
					if (transform2.GetComponent<Renderer>().material.shader.name != "iPhone/SolidTexture")
					{
						return false;
					}
				}
				else if (player.GetWeapon().GetWeaponType() == WeaponType.Sword)
				{
					...
					if (transform2.GetComponent<Renderer>().material.shader.name != "iPhone/SolidTextureBright")
					{
						return false;
					}
				}
