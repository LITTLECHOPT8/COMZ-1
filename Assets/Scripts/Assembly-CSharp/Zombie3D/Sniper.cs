		...
		public override void Fire(float deltaTime)
		{
			foreach (NearestEnemyInfo nearestEnemyInfo in nearestEnemyInfoList)
			{
				if (nearestEnemyInfo.transform != null)
				{
					...
					{
						ServerEventSystem.Send(1, new object[] { nearestEnemyInfo.transform.position.x, nearestEnemyInfo.transform.position.y, nearestEnemyInfo.transform.position.z } );
					}
					else if (GameApp.GetInstance().GetGameState().gameMode == GameMode.Vs)
					{
						ServerEventSystem.Send(10, new object[] { nearestEnemyInfo.transform.position }, false, true, true );
					}
				}
			}
