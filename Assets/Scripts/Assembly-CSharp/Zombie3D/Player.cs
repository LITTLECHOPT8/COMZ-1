...
namespace Zombie3D
{
	public class Player
	{
		...
		public void PlusVsKillCount()
		{
			...
			{
				ServerEventSystem.Send(8, new object[] { vs_combo_val }, true );
			}
			...
			if (!(bool)PhotonNetwork.room.customProperties["firstBlood"])
			{
				...
				ServerEventSystem.Send(12, null, true, false, true);
			}
		}
		...
		public void UpdateVSStatistic()
		{
			...
			table["killCount"] = vs_kill_count;
			table["deathCount"] = m_death_count;
			table["cashLoot"] = GameApp.GetInstance().GetGameState().loot_cash;
			table["vsCombo"] = vs_combo_val;
		...
		public virtual void Init()
		{
			...
			if (GameApp.GetInstance().GetGameState().gameMode == GameMode.Vs)
			{
				...
				tnet_user = PhotonNetwork.player;//tnetObj.Myself;
			}
			...
			SendNetMsg();
			charController.Move(new Vector3(0f, -10000f, 0f));
		}
		...
		public virtual void OnVSRebirth()
		{
			...
			ServerEventSystem.Send(7, null, false, false, true);
		...
		public void SendNetMsg()
		{
			...
			//else
			//{
				if (GameApp.GetInstance().GetGameState().gameMode != GameMode.Vs && GameApp.GetInstance().GetGameState().gameMode != GameMode.Coop)
				{
					return;
				}
				...
				table["weapon1"] = array2[0];
				table["weapon2"] = array2[1];
				table["weapon3"] = array2[2];

				table["weaponPara1"] = array3[0];
				table["weaponPara2"] = array3[1];
				table["weaponPara3"] = array3[2];

				table["weaponNum"] = 0;
				table["playerState"] = (int)playerState.GetStateType();
				table["playerBonusState"] = (int)playerBonusState.StateType;
		...
		public virtual void SetBonusState(PlayerBonusStateType type)
		{
			...
			if (GameApp.GetInstance().GetGameState().gameMode == GameMode.Coop)
			{
				ServerEventSystem.Send(3, new object[] { (int)playerBonusState.StateType } );
			}
		...
		public virtual void SetState(PlayerStateType type)
		{
			...
			if (GameApp.GetInstance().GetGameState().gameMode == GameMode.Coop)
			{
				ServerEventSystem.Send(2, new object[] { (int)playerState.GetStateType() } );
			}
			else if (GameApp.GetInstance().GetGameState().gameMode == GameMode.Vs && tnetObj != null)
			{
				ServerEventSystem.Send(2, new object[] { (int)playerState.GetStateType() } );
		...
		public virtual void OnHit(float damage)
		{
			...
			if (GameApp.GetInstance().GetGameState().gameMode == GameMode.Coop)
			{
				ServerEventSystem.Send(12, new object[] { damage, maxHp, hp } );
				if (No_Damage)
				{
					No_Damage = false;
				}
		...
		public virtual void OnInjuredWithUser(PhotonPlayer sender, float damage, int weapon_type)
		{
			if (!(HP > 0f))
			{
				return;
			...
			if (HP <= 0f)
			{
				...
				ServerEventSystem.Send(6, new object[] { sender.ID }, false, false, true );
				GameVSScene gameVSScene = GameApp.GetInstance().GetGameScene() as GameVSScene;
				Multiplayer multiplayer = gameVSScene.SFS_Player_Arr[sender] as Multiplayer;
				ServerEventSystem.Send(0, new object[] { multiplayer.nick_name + " FRAGGED " + GameApp.GetInstance().GetGameState().nick_name } );
		...
		public virtual void OnRebirth()
		{
			...
			if (GameApp.GetInstance().GetGameState().gameMode == GameMode.Coop)
			{
				...
				gameMultiplayerScene.SFS_Player_Arr[PhotonNetwork.player] = this;
		...
		{
			ServerEventSystem.Send(18, new object[] { m_multi_id } );
			is_real_dead = true;
		...
		public virtual void OnVsInjured(PhotonPlayer sender, float damage, int weapon_type)
		{
		}
		...
		public void SendNetUserChangeWeaponMsg(int index)
		{
			if (GameApp.GetInstance().GetGameState().gameMode == GameMode.Coop)
			{
				Debug.LogError("heloo ????");
				ServerEventSystem.Send(0, new object[] { index } );
				ExitGames.Client.Photon.Hashtable table = PhotonNetwork.player.customProperties;
				table["weaponNum"] = index;
				PhotonNetwork.player.SetCustomProperties(table);
			}
			else if (GameApp.GetInstance().GetGameState().gameMode == GameMode.Vs)
			{
				ServerEventSystem.Send(1, new object[] { index } );
				ExitGames.Client.Photon.Hashtable table = PhotonNetwork.player.customProperties;
				table["weaponNum"] = index;
				PhotonNetwork.player.SetCustomProperties(table);
				//SFSObject sFSObject = new SFSObject();
		...
		public virtual void OnVsPickUp(ItemType type, string index, int id)
		{
			OnPickUp(type);
			ServerEventSystem.Send(11, new object[] { id }, true);
		...
		public virtual bool OnMultiplayerPickUp(ItemType type, int id)
		{
			...
			ServerEventSystem.Send(11, new object[] { id } );
			return true;
		}
