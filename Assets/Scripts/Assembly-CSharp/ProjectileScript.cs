	...
	private void OnTriggerStay(Collider other)
	{
		...
		else if (other.gameObject.layer == 8 && GameApp.GetInstance().GetGameState().gameMode == GameMode.Vs)
		{
			if (!(this.player.PlayerObject == other.gameObject))
			{
				...
				if (damage > 0f)
				{
					ServerEventSystem.Send(5, new object[] { view.OwnerActorNr, damage, (int)gunType, view.transform.position }, true );
				}
			}
		}
				...
				GameVSScene gameVSScene = gameScene as GameVSScene;
				{
					foreach (Player value in gameVSScene.SFS_Player_Arr.Values)
					{
						...
						if (sqrMagnitude < num2 && damage > 0f)
						{
							...
							ServerEventSystem.Send(5, new object[] { value.PlayerObject.GetComponent<PhotonView>().OwnerActorNr, num3, (int)gunType, value.GetTransform().position }, true );
						}
					}
					return;
				}
		...
		else
		{
			...
			if (other.gameObject.layer == 8)
			{
				if (GameApp.GetInstance().GetGameState().gameMode == GameMode.Vs)
				{
					...
					if (damage > 0f)
					{
						ServerEventSystem.Send(5, new object[] { view.OwnerActorNr, damage, 12, view.transform.position }, true );
					}
				}
				return;
			}
