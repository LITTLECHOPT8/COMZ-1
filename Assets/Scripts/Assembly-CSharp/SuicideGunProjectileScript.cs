...
public class SuicideGunProjectileScript : MonoBehaviour
{
	...
	private void OnTriggerEnter(Collider other)
	{
			...
			GameVSScene gameVSScene = gameScene as GameVSScene;
			{
				foreach (Player value in gameVSScene.SFS_Player_Arr.Values)
				{
					...
					if (sqrMagnitude < num && damage > 0f)
					{
						...
						{
							ServerEventSystem.Send(5, new object[] { value.PlayerObject.GetComponent<PhotonView>().OwnerActorNr, num2, 13, value.GetTransform().position }, true );
						}
					}
				}
				return;
		...
		{
			foreach (Player item in ((GameMultiplayerScene)gameScene).SFS_Player_Arr.Values)
			{
