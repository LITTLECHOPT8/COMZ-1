...
public class CoopHallTUI : MonoBehaviour, TUIHandler
{
	...
	private void QuickMatch()
	{
		...
		if (info == null)
		{
			MsgBox.GetComponent<MsgBoxDelegate>().Show("NO ROOMS FOUND.", MsgBoxType.None);
			return;
		}
		IndicatorPanel.GetComponent<IndicatorTUI>().SetContent("SEARCHING...");
	...
	private bool CanJoinRoom(RoomInfo info)
	{
	...
	{
		OnJoinRoom((int)PhotonNetwork.room.customProperties["mapNumber"]);
	}

	private void OnJoinRoom(int map_id)
	{
		if (IndicatorPanel != null)
		{
			IndicatorPanel.GetComponent<IndicatorTUI>().Hide();
		...
		//{
			...
			FrameCamera.depth = -5f;
			GameApp.GetInstance().GetGameState().cur_net_map = SceneName.GetNetMapName(map_id);
	...
	private void CheckServerVersion()
	{
		...
		//OnServerVersionError();
		tnetObj = new TNetObject();
		TNetConnection.Connection = tnetObj;
		tnetObj.SetOnConnect(OnConnected);
		tnetObj.SetOnJoinedRoom(JoinRoom);
		...
		{
			tnetObj.SetOnConnectedToMaster(ConnectedToMaster);
			tnetObj.Connect();
		}
	}
