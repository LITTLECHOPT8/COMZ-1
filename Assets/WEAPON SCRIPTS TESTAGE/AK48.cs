...
namespace Zombie3D
{
    public class AK48 : Weapon
    {
        ...
        {
            return WeaponType.AssaultRifle;  // Correct type for assault rifles
        }
        ...
        {
            base.Init(); // Assuming there's a proper Init method in the parent class
            mAttributes.hitForce = 19;
            ...
            bulletsObjectPool.Init("AK48Bullets", rConf.bullets, 6, 1f);
            firelineObjectPool.Init("AK48Firelines", rConf.fireline, 4, 0.5f);
            sparksObjectPool.Init("AK48Sparks", rConf.hitparticles, 3, 0.22f);
        }
        ...
        public override void Fire(float deltaTime)
        {
            ...
            Ray ray = new Ray(fire_ori.transform.position, fire_ori.transform.TransformDirection(Vector3.forward));
            ...
            if (raycastHit.collider != null)
            {
                ...
                if (gameObject2.name.StartsWith("E_"))
                {
                    ...
                    if (enemyByID.GetState() != Enemy.DEAD_STATE)
                    {
                        ...
                        {
                            hitForce = ray.direction * mAttributes.hitForce,
                            damage = 0f
                        };

                        bool criticalAttack = UnityEngine.Random.Range(0, 100) < 70;
                        ...
                        else if ((float)UnityEngine.Random.Range(0, 100) < mAttributes.range)
                        {
                            enemyByID.OnHit(damageProperty, GetWeaponType(), criticalAttack, player);
                        }
                    }
